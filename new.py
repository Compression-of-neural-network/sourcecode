...
def train(epoch, model, Loss):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, cross_entropy = my_loss_function(output, target)
        Loss.append(loss)
        loss.backward()
        optimizer.step()
        # if batch_idx % 100 == 0:
        ...
        if batch_idx % 100 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)] loss: {:.6f} CrossEntropy: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset), 100. * batch_idx / len(train_loader),
                loss.item(), cross_entropy))
...
def test():
    ...
    for data, target in test_loader:
        ...
        t, cr = my_loss_function(output, target)
        test_loss += t.item()
    ...
    return test_loss, accuracy, crossentropy
...
if __name__ == '__main__':
    ...
    test_loss, accuracy, crossentropy = test()
    # log = pd.DataFrame(
    ...
    log = pd.DataFrame(
        [[CFG['batch_size'], CFG['learning_rate'], CFG['epoch'], CFG['lambda'], round(test_loss, 6),
          round(crossentropy, 6), accuracy]],
        columns=['batch_size', 'learning_rate', 'epoch', 'lambda', 'test_loss', 'crossentropy',
                 'accuracy'])
